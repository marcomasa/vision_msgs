...
#include <vision_msgs_rviz_plugins/detection_3d_with_pcl_array.hpp>
...
void Detection3DWithPclArrayDisplay::onInitialize()
{
    ...
    topic_property_->setValue("detection3_d_with_pcl_array");
    topic_property_->setDescription("Detection3DWithPclArray topic to subscribe to.");
...
void Detection3DWithPclArrayDisplay::update(float wall_dt, float ros_dt)
{
    m_marker_common->update(wall_dt, ros_dt);
    m_point_cloud_common->update(wall_dt, ros_dt);
}

void Detection3DWithPclArrayDisplay::reset()
{
...
void Detection3DWithPclArrayDisplay::updateEdge()
{
    only_edge_ = only_edge_property_->getBool();
    if (only_edge_)
    {
...
void Detection3DWithPclArrayDisplay::updateLineWidth()
{
    line_width = line_width_property_->getFloat();
    if (latest_msg)
    {
...
void Detection3DWithPclArrayDisplay::updateAlpha()
{
    alpha = alpha_property_->getFloat();
    if (latest_msg)
    {
...
void Detection3DWithPclArrayDisplay::updateColorConfigs()
{
    this->updateColorConfig();
}

void Detection3DWithPclArrayDisplay::updatePointCloud()
{
    show_pointcloud_ = show_pointcloud_property_->getBool();
    if (latest_msg)
    {
...
PLUGINLIB_EXPORT_CLASS(rviz_plugins::Detection3DWithPclArrayDisplay, rviz_common::Display)
